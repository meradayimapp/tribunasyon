...
<div class="profile-shell mx-auto">
    <section class="profile-header">
        ...
        <div class="profile-main">
            ...
            <p class="profile-date"><x-ui.icon name="calendar" />{{ $user->created_at->translatedFormat('F Y') }} tarihinde katıldı</p>
        </div>
    </section>
    @if($user->favoriteTeam)
        <section class="profile-section"><div class="section-heading"><h2>Favori takım</h2></div><a class="favorite-team" href="{{ route('teams.show', $user->favoriteTeam) }}"><x-team-logo :team="$user->favoriteTeam" size="md" /><span><strong>{{ $user->favoriteTeam->name }}</strong><small>Favori takım</small></span><x-ui.icon name="chevron-right" /></a></section>
    @endif
