...
>
    <div class="player-chat-heading">
        <div class="player-chat-title">
            ...
            <div><h2>{{ $player->name }} canlı sohbeti</h2><p>En yeni mesajlar her zaman üstte</p></div>
        </div>
    ...
    <div class="player-chat-log" x-ref="log" @scroll.passive="onScroll()" role="log" aria-live="polite" aria-label="{{ $player->name }} canlı sohbet mesajları">
        @forelse($messages as $message)
            <article class="player-chat-message role-{{ $message['user']['role'] }}" wire:key="player-message-{{ $message['id'] }}">
        ...
        @empty
            <div class="chat-empty"><span class="chat-empty-icon"><x-ui.icon name="comment" /></span><strong>Canlı sohbeti sen başlat.</strong><span>{{ $player->name }} hakkında ilk mesajı yaz.</span></div>
        @endforelse
    ...
    @auth
        <div class="player-chat-composer-shell" x-data="{ length: 0 }" @player-chat-sent.window="length = 0">
            <form wire:submit="send" class="player-chat-composer">
                ...
                <div class="player-chat-input-wrap">
                    ...
                    <textarea id="player-chat-body-{{ $player->id }}" wire:model="body" rows="1" maxlength="500" placeholder="Canlı sohbete bir mesaj yaz..." @input="length = $event.target.value.length; $event.target.style.height = 'auto'; $event.target.style.height = Math.min($event.target.scrollHeight, 112) + 'px'" @keydown.enter="if (!$event.shiftKey) { $event.preventDefault(); $wire.send() }"></textarea>
                    <div class="player-chat-input-meta"><span>Enter gönderir · Shift+Enter yeni satır</span><span x-text="length + '/500'">0/500</span></div>
                </div>
                <button type="submit" wire:loading.attr="disabled" wire:target="send" aria-label="Mesajı gönder">
                    <span wire:loading.remove wire:target="send"><x-ui.icon name="send" /></span>
                    <span wire:loading wire:target="send" class="mini-loader"></span>
                </button>
            </form>
            @error('body')<div class="chat-error">{{ $message }}</div>@enderror
        </div>
    @else
        <div class="chat-login-prompt"><span><strong>Canlı sohbete katıl.</strong> Mesaj yazmak için hesabına giriş yap.</span><a href="{{ route('login') }}">Giriş yap</a></div>
    @endauth
</section>
